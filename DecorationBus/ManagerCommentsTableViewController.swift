//
//  ManagerCommentsTableViewController.swift
//  DecorationBus
//
...
class ManagerCommentsTableViewController: UITableViewController, MWPhotoBrowserDelegate {
    ...
    var _comments: Array<ManagerComment> = Array<ManagerComment>()
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = self._manager.name //设置导航栏标题
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCellWithIdentifier("managercommentcell", forIndexPath: indexPath) as! ManagerCommentTableViewCell
        ...
        */
        ...
        for i in 0..<commentsData.thumbnails.count {
            ...
            cell.configureImageGesture(i, target: self, action: #selector(ManagerCommentsTableViewController.imageTapped(_:)), tag: viewTag)
        }
    ...
    */
    ...
    func requestManagerComments(counter: Int, target: UInt) -> Array<ManagerComment> {
        ...
        do {
            ...
            return parseManagerComments(data)
        }catch let error as NSError{
            print("网络异常--请求公司评论信息失败：" + error.localizedDescription)
        }
        
        return Array<ManagerComment>()
    }
    
    // 解析请求到的JSON数据
    func parseManagerComments(jsonData: NSData) -> Array<ManagerComment> {
        do {
            ...
            var requestedManagerComments = Array<ManagerComment>()
            for item in items {
                ...
                requestedManagerComments.append(comment)
            }
            
            //服务端返回的JSON有误时仅打印一条信息
            if(itemNum != requestedManagerComments.count) {
                print("Warning: items number mismatch in json!")
            }
            
            return requestedManagerComments
        }catch let error as NSError {
    ...
    func tableFooterRefresh() {
        ...
        for item in requestedManagerComments {
...
extension ManagerCommentsTableViewController: CommentTableViewControllerDelegate {
