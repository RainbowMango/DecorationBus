        ...
        if (_selected != selected)
        {
            if (selected) {
                ...
                if ([self.delegate respondsToSelector:@selector(agGridItemCanSelect:)])
                {
                    ...
                    {
                        NSString *str;
                        str = [NSString stringWithFormat:@"一次最多只能选择%lu张图片哦", (unsigned long)numberOfSelectedGridItems];
                        UIAlertView *myAlertView;
                        myAlertView=[[UIAlertView alloc] initWithTitle:nil message:str delegate:self cancelButtonTitle:@"我知道了" otherButtonTitles:nil, nil];
                        [myAlertView show];
                        //[myAlertView release];
                        return;
                    }
